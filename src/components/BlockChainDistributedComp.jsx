...
const BlockChainDistributedComp = (props) => {
    ...
    async function keyup(e) {
        ...
        await API.PostRequestApi(CONSTANTS.API_ENDPOINT.UPDATE_DISTRIBUTED_BLOCK +props.peer+'/'+props.index+'/',toSend, CONSTANTS.API_TIMEOUT_TYPE.SLOW_TIMEOUT).then((data)=>{props.changeCurState(props.peer,props.index,data)}).catch(err=>console.log(err));
    ...
    async function mine(){
        ...
        props.setLoading(true);
        await API.PostRequestApi(CONSTANTS.API_ENDPOINT.MINE_DISTRIBUTED_BLOCK +props.peer+'/'+props.index+'/',toSend, CONSTANTS.API_TIMEOUT_TYPE.SLOW_TIMEOUT).then((data)=>{props.changeCurState(props.peer,props.index,data)}).catch(err=>console.log(err));
        props.setLoading(false);
    ...
    return (
        <div>
        <br />
            <div style={{background: typeof hashvalue != "undefined"?(hashvalue.substring(0, 4) === '0000' ? 'rgb(223, 240, 216)' : 'rgb(250, 220, 220)'):'rgb(250, 220, 220)', padding: '35px', borderRadius: '5px'}}>
                <InputGroup size="sm">
...
export default BlockChainDistributedComp